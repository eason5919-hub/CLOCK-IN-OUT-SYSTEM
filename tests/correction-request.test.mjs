...
test("correction requests save Malaysia time and clock-out approvals target open rows", async () => {
  ...
  assert.match(script, /clockOut: row\.clockOut \? "edited" : ""/);
  assert.match(script, /row\.source === "admin_adjustment" && !approved\.length/);
  ...
  assert.match(adminHtml, /\.editableCell\.correctedTime\.manualEdit/);
